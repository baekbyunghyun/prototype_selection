...
class HyperRectangleSelectorV30:
    def __init__(self, X=None, y=None, tau=1.0):
        self.X = X
        self.y = y
        self.tau = tau
    ...
    def get_nearest_prototype(self, valid_prototypes, x):
        nearest_prototype = None

        for prototype in valid_prototypes:
            if nearest_prototype is None:
                nearest_prototype = prototype

                continue

            sum_distance = 0
            sum_nearest_distance = 0
            for x_attr, prototype_attr, nearest_prototype_attr in zip(x, prototype.x_mid, nearest_prototype.x_mid):
                sum_distance += abs(x_attr - prototype_attr)
                sum_nearest_distance += abs(x_attr - nearest_prototype_attr)

            if sum_distance < sum_nearest_distance:
                nearest_prototype = prototype
    ...
    def greedy(self):
        ...
        for x, y in zip(self.X, self.y):
            ...
            valid_prototypes = list()
            for hr in self.hyper_rectangles:
                if self.is_include(hr, x, y):
                    valid_prototypes.append(hr)

            if len(valid_prototypes) > 0:
                is_covered = True

                nearest_prototype = self.get_nearest_prototype(valid_prototypes, x)
                nearest_prototype.set_covered_data(index, x)
